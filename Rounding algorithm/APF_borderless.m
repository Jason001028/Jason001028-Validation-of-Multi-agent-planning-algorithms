    ...
    for i = 1:pursuers_num
        ...
        for j = (pursuers_num+1):agents_sum
            if agents(j).active
                ...
                diff(1) = mod(diff(1) + L/2, L) - L/2;
                diff(2) = mod(diff(2) + L/2, L) - L/2;
                dist = norm(diff);
                is_targeted = false;
                for k = 1:pursuers_num
                    if k ~= i
                        diff_k = agents(k).pos - agents(j).pos;
                        diff_k(1) = mod(diff_k(1) + L/2, L) - L/2;
                        diff_k(2) = mod(diff_k(2) + L/2, L) - L/2;
                        if norm(diff_k) < dist
                            is_targeted = true;
                            break;
                        end
